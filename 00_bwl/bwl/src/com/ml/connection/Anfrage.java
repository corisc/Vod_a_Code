...
public class Anfrage {
  ...
  private final int port = BasicEnvConfiguration.AUSSENSTELLE_PORT;
  ...
  public String xmlHolenVonServer(String server) {
    String result = null;
    String serverm = "81.169.154.70";
    String serverh = "81.169.154.100";
    if (server.equals(serverm) || server.equalsIgnoreCase("a")) {
      result = "<ABSATZDATENSATZ>" + "\n" + "<SUPER>11.0</SUPER>" + "\n"
          + "<DIESEL>22.0</DIESEL>" + "\n" + "</ABSATZDATENSATZ>";
    }
    if (server.equals(serverh) || server.equalsIgnoreCase("b")) {
      result = "<ABSATZDATENSATZ>" + "\n" + "<SUPER>44.0</SUPER>" + "\n"
          + "<DIESEL>77.0</DIESEL>" + "\n" + "</ABSATZDATENSATZ>";
    }
    if(result == null){
      System.out.println("Der uebergebene Server existiert nicht: " + server);
      result = "Die Serveradresse existiert nicht.";
    }
    
    return result;
  }

  public String xmlHolen(String server) {
    String result = "";
    String servermapped = "81.169.154.70";
    if (server.contains("mleue") || server.contains("70")) {
      this.url = "/bwl/xml/aral.xml";
      System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!");
    } else {
      this.url = "/bwl/xml/aral.xml";
    }
    result = this.durchfuehren(servermapped, this.port, this.protocol,
        this.url, this.proxyUrl, this.proxyPort, this.requestProperties,
  ...
  public String durchfuehren(String host, int port, String protocol,
      String url, String proxyUrl, int proxyPort, Properties requestProperties,
    ...
    try {
      ...
      // info
      if (true) {
        System.out.println("sending this request: " + hostConfig.getHostURL()
      ...
      if (isQuoteRequestFlag) {
        System.out.println("Aufrufzeit Quote Request: "
            + (timeStop - timeStart));
      }
  ...
  // System.out.println("!!!" + anfrage.durchfuehren("127.0.0.1", 80, "http",
  // "", null, 3128, null, "", false));
  // }
}
