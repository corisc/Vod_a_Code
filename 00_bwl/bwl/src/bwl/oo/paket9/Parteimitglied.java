package bwl.oo.paket9;

public class Parteimitglied {
	private String vorname;
	private String nachname;
	private String adresse;
	private String partei;
	
	public Parteimitglied(String vorname, String nachname, String adresse) {
		this.vorname = vorname;
		this.nachname = nachname;
		this.adresse = adresse;
	}
	public Parteimitglied(String vorname, String nachname, String adresse, String partei) {
		this.vorname = vorname;
		this.nachname = nachname;
		this.adresse = adresse;
		this.partei = partei;
	}

	public String liefereVorname() {
		return vorname;
	}

	public void setzeVorname(String vorname) {
		this.vorname = vorname;
	}

	public String liefereNachname() {
		return nachname;
	}

	public void setzeNachname(String nachname) {
		this.nachname = nachname;
	}

	public String liefereAdresse() {
		return adresse;
	}

	public void setzeAdresse(String adresse) {
		this.adresse = adresse;
	}
	
	public void setzePartei(String partei) {
		this.partei = partei;
	}

	public String lieferePartei() {
		return partei;
	}
}
